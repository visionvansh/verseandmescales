// components/profile/SeekingTab.tsx
"use client";
import { useState } from "react";
import { motion } from "framer-motion";
import { User } from "@/components/profile/data/mockProfileData";
import UserHoverCard from "./UserHoverCard";
import { FaStar, FaChalkboardTeacher, FaGraduationCap, FaUserGraduate, FaCheckCircle } from "react-icons/fa";
import { useRouter } from "next/navigation";

interface SeekingTabProps {
  users: User[];
}

export default function SeekingTab({ users }: SeekingTabProps) {
  ...
  if (users.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">Start following tutors and learners to see them here.</p>
      </div>
    );
  }
        ...
        {users.map((user, index) => (
          ...
          >
            ...
            <div className="relative p-5">
              ...
              <motion.button
                className="w-full py-2 bg-gray-800/50 hover:bg-gray-800 border border-red-500/30 rounded-lg text-gray-300 text-sm font-semibold transition-all flex items-center justify-center gap-2"
