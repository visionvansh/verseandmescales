// /api/user/recovery/send-verification/route.ts
    ...
    html: `
      ...
      <html>
        <head>
          ...
          <title>VerseAndMe Scales - Recovery Email Verification</title>
        </head>
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ✅ Only support email now (phone removed)
    if (type !== 'email') {
    ...
    // Check rate limiting
    const rateLimitKey = `recovery:verify:ratelimit:${user.id}:${type}`;
    ...
    // ✅ Send verification code using @/utils/email.ts with VerseAndMe Scales design
    try {
      const emailTemplate = getRecoveryVerificationEmailTemplate(code);
      
      await sendEmail({
        to: value,
        subject: 'VerseAndMe Scales - Verify Your Recovery Email',
        text: emailTemplate.text,
        html: emailTemplate.html
      });

      console.log('[Recovery Email Sent] ✅ Verification code sent to:', value);
    } catch (sendError) {
    ...
    // Update rate limit
    await redis.set(rateLimitKey, (parseInt(rateLimitCount || '0') + 1).toString(), 'EX', 900); // 15 min

    // Log the activity
    await prisma.userActivityLog.create({
      data: {
        userId: user.id,
        action: 'recovery_verification_sent',
        description: `Verification code sent to ${type}: ${value}`,
