// app/users/studio/course-builder/page.tsx
"use client";
...
// ✅ Import studio-specific modal
import StudioSubmissionModal from "@/components/studio/StudioSubmissionModal";
...
const CourseBuilder = () => {
  ...
  // ✅ Add state for custom homepage file
  const [customHomepageFile, setCustomHomepageFile] = useState<string | null>(null);
  ...
  // ✅ Load custom homepage info
  const loadCourseInfo = async (courseId: string) => {
    try {
      const response = await fetch(`/api/studio/course-info?courseId=${courseId}`);
      if (response.ok) {
        const data = await response.json();
        setCustomHomepageFile(data.customHomepageFile);
      }
    } catch (error) {
  ...
  // ✅ Update loadData to include course info
  const loadData = async (courseId: string) => {
    setIsLoading(true);
    try {
      await Promise.all([
        loadCourseData(courseId),
        loadCourseInfo(courseId), // ✅ Add this
      ]);
    } catch (error) {
  ...
  return (
    <LazyMotion features={domAnimation}>
            ...
            <div className="relative p-4 sm:p-5">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <m.button
                    onClick={() => router.push(`/users/studio/card-customisation?courseId=${courseId}`)}
      ...
      <StudioSubmissionModal
        isOpen={showSubmissionModal}
        onClose={() => setShowSubmissionModal(false)}
        courseId={courseId || ''}
        moduleData={courseData}
        customHomepageFile={customHomepageFile || ''}
